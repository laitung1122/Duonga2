...
if not ExecutorSupport then
	print(
		"[mspaint] Đã dừng tải, vui lòng sử dụng chuỗi tải chính thức cho Dương Api. (LỖI: ExecutorSupport == nil)"
	)
	return
end
if getgenv().mspaint_loaded then
	print("[Dương-Api] Đã dừng tải. (ERROR: Đã chạy main)")
	return
end
...
if
	Players.LocalPlayer
	and Players.LocalPlayer.PlayerGui:FindFirstChild("LoadingUI")
	and Players.LocalPlayer.PlayerGui.LoadingUI.Enabled
then
	print("[Dương Api] Đang chờ tải trò chơi...")
	repeat
		task.wait()
	until not Players.LocalPlayer.PlayerGui:FindFirstChild("LoadingUI") and true
		or not Players.LocalPlayer.PlayerGui.LoadingUI.Enabled
end
...
local remotesFolder = if not isFools
	then ReplicatedStorage:WaitForChild("RemotesFolder")
	else ReplicatedStorage:WaitForChild("EntityInfo")
...
local controlModule = if ExecutorSupport["require"]
	then require(playerScripts:WaitForChild("PlayerModule"):WaitForChild("ControlModule"))
	else nil
...
--// Captions \\--
local _mspaint_custom_captions = Instance.new("ScreenGui")
do
	...
	function Script.Functions.Captions(caption: string)
		...
		if _mspaint_custom_captions.Parent == ReplicatedStorage then
			_mspaint_custom_captions.Parent = gethui() or game:GetService("CoreGui") or playerGui
		end
...
function Script.Functions.UpdateRPC()
	...
	BloxstrapRPC.SetRichPresence({
		...
		state = roomNumberPrefix
			.. prettifiedRoomNumber
			.. " ("
			.. (PrettyFloorName[floor.Value] and PrettyFloorName[floor.Value] or ("The " .. floor.Value))
			.. ")",
		largeImage = {
...
do
	...
	function Script.Functions.Notifs.Doors.Notify(unsafeOptions)
		assert(
			typeof(unsafeOptions) == "table",
			"Expected a table as options argument but got " .. typeof(unsafeOptions)
		)
		if not mainUI then
			return
		end
		...
		task.spawn(function()
			...
			TweenService
				:Create(acheivement.Frame.Glow, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
					ImageTransparency = 1,
				})
				:Play()
	...
	function Script.Functions.Log(options, condition: boolean | nil)
		...
		elseif Options.NotifyStyle.Value == "Doors" then
			if not condition and typeof(condition) == "boolean" then
				return
			end
...
do
	function Script.Functions.ESP(args: ESP)
		if not args.Object then
			return Script.Functions.Warn("ESP Object is nil")
		end
	...
	function Script.Functions.DoorESP(room)
		...
		if door then
			...
			Script.FeatureConnections.Door[doorIdx] = door:GetAttributeChangedSignal("Opened"):Connect(function()
				if doorEsp then
					doorEsp.SetText(string.format("Cửa %s [Đã mở]", doorNumber))
				end
				if Script.FeatureConnections.Door[doorIdx] then
					Script.FeatureConnections.Door[doorIdx]:Disconnect()
				end
		...
		elseif child.Name == "MinesAnchor" then
			...
			if sign and sign:FindFirstChild("TextLabel") then
				Script.Functions.ESP({
					Type = "Objective",
					Object = child,
					Text = string.format("Trạm %s", sign.TextLabel.Text),
					Color = Options.ObjectiveEspColor.Value,
				})
			end
		elseif child.Name == "WaterPump" then
			...
			if wheel and (onFrame and onFrame.Visible) then
				...
				local pumpEsp = Script.Functions.ESP({
					...
					OnDestroy = function()
						if Script.FeatureConnections.Pump[pumpIdx] then
							Script.FeatureConnections.Pump[pumpIdx]:Disconnect()
						end
					end,
				})
	...
	function Script.Functions.PlayerESP(player: Player)
		...
		Script.FeatureConnections.Player[player.Name] = player.Character.Humanoid.HealthChanged:Connect(
			function(newHealth)
				if newHealth > 0 then
					playerEsp.SetText(string.format("%s [%.1f]", player.DisplayName, newHealth))
				else
					if Script.FeatureConnections.Player[player.Name] then
						Script.FeatureConnections.Player[player.Name]:Disconnect()
					end
					playerEsp.Destroy()
				end
			end
	...
	function Script.Functions.GuidingLightEsp(guidance)
		...
		guidance.AncestryChanged:Connect(function()
			if not guidance:IsDescendantOf(workspace) then
				if Script.Temp.Guidance[guidance] then
					Script.Temp.Guidance[guidance] = nil
				end
				if part then
					part:Destroy()
				end
...
do
	...
	function Script.Functions.ItemCondition(item)
		return item:IsA("Model")
			and (item:GetAttribute("Pickup") or item:GetAttribute("PropType"))
			and not item:GetAttribute("FuseID")
	end
	...
	function Script.Functions.ChildCheck(child)
		...
		if Script.Functions.PromptCondition(child) then
			task.defer(function()
				...
				if not child:GetAttribute("Enabled") then
					child:SetAttribute("Enabled", child.Enabled)
				end
		...
		if child:IsA("Model") then
			...
			elseif child.Name == "GiggleCeiling" and Toggles.AntiGiggle.Value then
				child:WaitForChild("Hitbox", 5).CanTouch = false
			elseif
				(child:GetAttribute("LoadModule") == "DupeRoom" or child:GetAttribute("LoadModule") == "SpaceSideroom")
				and Toggles.AntiDupe.Value
			then
			...
			if
				(isHotel or isFools)
				and (child.Name == "ChandelierObstruction" or child.Name == "Seek_Arm")
				and Toggles.AntiSeekObstructions.Value
			then
			...
			if isFools then
				if Toggles.FigureGodmodeFools.Value and child.Name == "FigureRagdoll" then
					for i, v in pairs(child:GetDescendants()) do
						if v:IsA("BasePart") then
							if not v:GetAttribute("Clip") then
								v:SetAttribute("Clip", v.CanCollide)
							end
							...
							task.spawn(function()
								repeat
									task.wait()
								until latestRoom.Value == 50 or latestRoom.Value == 100
								task.wait(5)
								v.CanCollide = false
							end)
						end
		...
		elseif child:IsA("BasePart") then
			...
			if isMines then
				if
					Toggles.AntiBridgeFall.Value
					and child.Name == "PlayerBarrier"
					and child.Size.Y == 2.75
	...
	function Script.Functions.IsPromptInRange(prompt: ProximityPrompt)
		return Script.Functions.DistanceFromCharacter(
	...
	function Script.Functions.GetNearestAssetWithCondition(condition: () -> ())
		...
		for _, room in pairs(workspace.CurrentRooms:GetChildren()) do
			if not room:FindFirstChild("Assets") then
				continue
			end
	...
	function Script.Functions.GetAllPromptsWithCondition(condition)
		assert(
			...
			"Expected a function as condition argument but got " .. typeof(condition)
		)
	...
	function Script.Functions.GetNearestPromptWithCondition(condition)
		...
		for _, prompt in pairs(prompts) do
			local promptParent = prompt:FindFirstAncestorWhichIsA("BasePart")
...
do
	...
	function Script.Functions.DeleteSeek(collision: BasePart)
		...
		task.spawn(function()
			...
			if
				collision:IsDescendantOf(workspace)
				and (collision.Parent and collision.Parent.Name == "TriggerEventCollision")
			then
				...
				if fireTouch then
					...
					repeat
						if collision:IsDescendantOf(workspace) then
							fireTouch(collision, rootPart, 1)
						end
						task.wait()
						if collision:IsDescendantOf(workspace) then
							fireTouch(collision, rootPart, 0)
						end
						task.wait()
	...
	function Script.Functions.AvoidEntity(value: boolean, oldNoclip: boolean)
		if not rootPart or not collision then
			return
		end
...
do
	function Script.Functions.GenerateAutoWardrobeExclusions(targetWardrobePrompt: ProximityPrompt)
		if not workspace.CurrentRooms:FindFirstChild(currentRoom) then
			return { targetWardrobePrompt.Parent }
		end
		...
		if workspace.CurrentRooms[currentRoom]:FindFirstChild("Assets") then
			for _, asset in pairs(workspace.CurrentRooms[currentRoom].Assets:GetChildren()) do
				if asset.Name == "Pillar" then
					table.insert(ignore, asset)
				end
			end
		end
	...
	function Script.Functions.AutoWardrobe(child, index: number | nil) -- child = entity, ty upio
		...
		local targetWardrobeChecker = function(prompt)
			...
			if not prompt.Parent:FindFirstChild("HiddenPlayer") then
				return false
			end
			if prompt.Parent:FindFirstChild("Main") and prompt.Parent.Main:FindFirstChild("HideEntityOnSpot") then
				if prompt.Parent.Main.HideEntityOnSpot.Whispers.Playing == true then
					return false
				end -- Hide
			end

			return prompt.Name == "HidePrompt"
				and (prompt.Parent:GetAttribute("LoadModule") == "Wardrobe" or prompt.Parent:GetAttribute("LoadModule") == "Bed" or prompt.Parent.Name == "Rooms_Locker")
				and not prompt.Parent.HiddenPlayer.Value
				and (
					Script.Functions.DistanceFromCharacter(prompt.Parent)
					< prompt.MaxActivationDistance * Options.PromptReachMultiplier.Value
				)
		end
		local targetWardrobePrompt = Script.Functions.GetNearestPromptWithCondition(targetWardrobeChecker)
		local getPrompt = function()
			if
				not targetWardrobePrompt
				or Script.Functions.DistanceFromCharacter(
						targetWardrobePrompt:FindFirstAncestorWhichIsA("Model"):GetPivot().Position
					)
					> 15
			then
				repeat
				...
				until targetWardrobePrompt ~= nil
					or character:GetAttribute("Hiding")
					or (not Toggles.AutoWardrobe.Value or not alive or not child or not child:IsDescendantOf(workspace))
					or Library.Unloaded

				if
					(not Toggles.AutoWardrobe.Value or not alive or not child or not child:IsDescendantOf(workspace))
					or Library.Unloaded
				then
					return
				end
		...
		if not Toggles.AutoWardrobe.Value or not alive or Library.Unloaded then
			return
		end
		...
		local isSafeCheck = function(addMoreDist)
			...
			for _, entity in pairs(Script.Temp.AutoWardrobeEntities) do
				...
				local inView = Script.Functions.IsInViewOfPlayer(
					entity.PrimaryPart,
					distanceEntity + (addMoreDist == true and 15 or 0),
					exclusion
				)
				local isClose = Script.Functions.DistanceFromCharacter(entity:GetPivot().Position)
					< distanceEntity + (addMoreDist == true and 15 or 0)
		...
		local waitForSafeExit
		waitForSafeExit = function()
			if child.Name == "A120" then
				repeat
					task.wait()
				until not child:IsDescendantOf(workspace)
					or (child.PrimaryPart and child.PrimaryPart.Position.Y < -10)
					or (not alive or not character:GetAttribute("Hiding"))
			else
				local didPlayerSeeEntity = false
				task.spawn(function()
					repeat
						task.wait()
						if not alive or not child or not child:IsDescendantOf(workspace) then
							break
						end

						if
							character:GetAttribute("Hiding")
							and Script.Functions.IsInViewOfPlayer(child.PrimaryPart, distance, exclusion)
						then
				...
				repeat
						...
						if isSafe == true and not child:IsDescendantOf(workspace) then
							task.spawn(function()
								Script.Functions.Log({
									...
									LinoriaMessage = "["
										.. NotifPrefix
										.. "] đang ra khỏi tủ, quái đã bị xóa.",
								}, Toggles.AutoWardrobeNotif.Value)
							end)
					...
					if not alive then
						if Toggles.AutoWardrobeNotif.Value then
							Script.Functions.Log("[" .. NotifPrefix .. "] Đang dừng lại (Bạn đã hẹo).")
						end
						task.spawn(function()
							Script.Functions.Log({
								Title = NotifPrefix,
		...
		local hide = function()
			if character:GetAttribute("Hiding") and rootPart.Anchored then
				return false
			end
			...
			if atempts > maxAtempts or not alive then
				return false
			end
			return true
		end
		...
		if child.Name == "AmbushMoving" then
			...
			repeat
				...
				until (
						IsMoving == true
						and Script.Functions.DistanceFromCharacter(child:GetPivot().Position) <= distance
					) or (not child or not child:IsDescendantOf(workspace))
			...
			repeat
				task.wait()
			until isSafeCheck(true, true) == false
	...
	function Script.Functions.SolveBreakerBox(breakerBox)
		...
		if Options.AutoBreakerSolverMethod.Value == "Legit" then
			...
			if Script.Connections["Reset"] then
				Script.Connections["Reset"]:Disconnect()
			end
			if Script.Connections["Code"] then
				Script.Connections["Code"]:Disconnect()
			end
			...
			if code:FindFirstChild("Frame") then
				...
				Script.Connections["Reset"] = correct:GetPropertyChangedSignal("Playing"):Connect(function()
					if correct.Playing then
						table.clear(Script.Temp.UsedBreakers)
					end
				end)
...
do
	...
	function Script.Functions.Minecart.Pathfind(room: Model, lastRoom: number)
		if not (lastRoom >= 40 and lastRoom <= 49) and not (lastRoom >= 95 and lastRoom <= 100) then
			return
		end
		...
		if numOfNodes <= 1 then
			return
		end --This is literally impossible but... umm. acutally, yea why not.
	...
	function Script.Functions.Minecart.NodeDestroy(roomNum: number)
		...
		print(
			string.format(
				"[NodeDestroy] Task completed, remaining: Real nodes: %d | Fake nodes %s",
				#realNodes,
				#fakeNodes
			)
		)
	...
	function Script.Functions.Minecart.DrawNodes()
		...
		for idx, path: tPathfind in ipairs(MinecartPathfind) do
			if path.esp and pathESP_enabled then
				continue
			end -- if status is unchanged.
...
do
	...
	function Script.Functions.SetupCharacterConnection(newCharacter)
		...
		if character then
			...
			Script.FeatureConnections.Character["ChildAdded"] = character.ChildAdded:Connect(function(child)
				if child:IsA("Tool") and child.Name:match("LibraryHintPaper") then
					...
					if
						Toggles.AutoLibrarySolver.Value
						and tonumber(code)
						and Script.Functions.DistanceFromCharacter(padlock) <= Options.AutoLibraryDistance.Value
					then
			...
			Script.FeatureConnections.Character["CanJump"] = character
				:GetAttributeChangedSignal("CanJump")
				:Connect(function()
			...
			Script.FeatureConnections.Character["Crouching"] = character
				:GetAttributeChangedSignal("Crouching")
				:Connect(function()
			...
			Script.FeatureConnections.Character["Hiding"] = character
				:GetAttributeChangedSignal("Hiding")
				:Connect(function()
							...
							if obj.Value == character then
								task.spawn(function()
									...
									for _, part in pairs(obj.Parent:GetChildren()) do
										if not part:IsA("BasePart") or part.Name:match("Collision") then
											continue
										end
									...
									until not character:GetAttribute("Hiding")
										or not Toggles.TranslucentHidingSpot.Value
			...
			Script.FeatureConnections.Character["Oxygen"] = character
				:GetAttributeChangedSignal("Oxygen")
				:Connect(function()
					if character:GetAttribute("Oxygen") < 100 and Toggles.NotifyOxygen.Value then
						if ExecutorSupport["firesignal"] then
							firesignal(
								...
								string.format("Oxygen: %.1f", character:GetAttribute("Oxygen"))
							)
						else
		...
		if humanoid then
			...
			Script.FeatureConnections.Humanoid["Move"] = humanoid
				:GetPropertyChangedSignal("MoveDirection")
				:Connect(function()
					if
						Toggles.FastClosetExit.Value
						and humanoid.MoveDirection.Magnitude > 0
						and character:GetAttribute("Hiding")
					then
						remotesFolder.CamLock:FireServer()
					end
				end)

			Script.FeatureConnections.Humanoid["Jump"] = humanoid
				:GetPropertyChangedSignal("JumpHeight")
				:Connect(function()
		...
		if rootPart then
			...
			Script.FeatureConnections.RootPart["Anchored"] = rootPart
				:GetPropertyChangedSignal("Anchored")
				:Connect(function()
					local lastAnchoredDelta = os.time() - Script.Lagback.LastAnchored

					if
						rootPart.Anchored
						and Toggles.LagbackDetection.Value
						and Toggles.SpeedBypass.Value
						and not Script.Lagback.Detected
					then
						...
						if Script.Lagback.Anchors >= 2 and lastAnchoredDelta <= Script.Lagback.Threshold then
							...
							Script.Functions.Alert({
								Title = "Fix bug",
								Description = "Đang sửa lỗi văng map...",
							})
							...
							if rootPart.Anchored == true then
								...
								repeat
									task.wait()
								until rootPart.Anchored == false or (os.time() - lastCheck) > 5
							end
							task.spawn(function()
								lastCheck = os.time()
							...
							repeat
								task.wait()
							until cframeChanged or (os.time() - lastCheck) > 5
							if rootPart.Anchored == true then
								lastCheck = os.time()
								repeat
									task.wait()
		...
		if isMines then
			if character then
				Script.Connections["AnticheatBypassTheMines"] = character
					:GetAttributeChangedSignal("Climbing")
					:Connect(function()
				...
				if child:IsA("Tool") and child.Name:match("LibraryHintPaper") then
					...
					if Toggles.NotifyPadlock.Value and count < 5 then
						Script.Functions.Alert({
							...
							Reason = (
								tonumber(code) and "Solved the library padlock code"
								or "You are still missing some books"
							),
						})
...
local PlayerGroupBox = Tabs.Main:AddLeftGroupbox("Người dùng")
do
...
local ReachGroupBox = Tabs.Main:AddLeftGroupbox("Reach")
do
...
local AutomationGroupBox = Tabs.Main:AddRightGroupbox("Tự động")
do
...
local MiscGroupBox = Tabs.Main:AddRightGroupbox("Tính năng khác")
do
...
local AntiEntityGroupBox = Tabs.Exploits:AddLeftGroupbox("Chống quái")
do
...
local TrollingGroupBox = Tabs.Exploits:AddLeftGroupbox("Trolling")
do
...
local BypassGroupBox = Tabs.Exploits:AddRightGroupbox("Bỏ qua")
do
...
do
	local ESPTab = ESPTabBox:AddTab("định vị")
	do
	...
	local ESPSettingsTab = ESPTabBox:AddTab("Cài đặt")
	do
...
local AmbientGroupBox = Tabs.Visuals:AddLeftGroupbox("Môi trường xung quanh")
do
...
do
	local NotifyTab = NotifyTabBox:AddTab("Cảnh báo")
	do
	...
	local NotifySettingsTab = NotifyTabBox:AddTab("Cài đặt")
	do
...
local SelfGroupBox = Tabs.Visuals:AddRightGroupbox("Bản thân")
do
...
task.spawn(function()
	if isHotel then
		local Hotel_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("Anti-Entity")
		do
		...
		local Hotel_BypassGroupBox = Tabs.Floor:AddLeftGroupbox("Bypass")
		do
		...
		local Hotel_ModifiersGroupBox = Tabs.Floor:AddRightGroupbox("Modifiers")
		do
	...
	elseif isMines then
		local Mines_MovementGroupBox = Tabs.Floor:AddLeftGroupbox("Chuyển động")
		do
		...
		local Mines_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("Chống quái")
		do
		...
		local Mines_AutomationGroupBox = Tabs.Floor:AddRightGroupbox("Tự động")
		do
		...
		local Mines_BypassGroupBox = Tabs.Floor:AddRightGroupbox("Bypass")
		do
		...
		local Mines_VisualGroupBox = Tabs.Floor:AddRightGroupbox("Visuals")
		do
		...
		Toggles.TheMinesAnticheatBypass:OnChanged(function(value)
			if value then
				...
				if bypassed and not Script.FakeRevive.Enabled then
					...
					Options.SpeedSlider:SetMax(
						Toggles.SpeedBypass.Value and 45 or (Toggles.EnableJump.Value and 3 or 7)
					)
		...
		Toggles.AntiBridgeFall:OnChanged(function(value)
			if value then
				for _, room in pairs(workspace.CurrentRooms:GetChildren()) do
					if not room:FindFirstChild("Parts") then
						continue
					end
					...
					for _, bridge in pairs(room.Parts:GetChildren()) do
						if bridge.Name == "Bridge" then
							for _, barrier in pairs(bridge:GetChildren()) do
								if
									not (
										barrier.Name == "PlayerBarrier"
										and barrier.Size.Y == 2.75
										and (barrier.Rotation.X == 0 or barrier.Rotation.X == 180)
									)
								then
									continue
								end
	...
	elseif isBackdoor then
		local Backdoors_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("Anti-Entity")
		do
		...
		local Backdoors_VisualGroupBox = Tabs.Floor:AddRightGroupbox("Visual")
		do
		...
		Toggles.AntiHasteJumpscare:OnChanged(function(value)
			...
			if not internal_temp_mspaint then
				internal_temp_mspaint = Instance.new("Folder", clientRemote)
				internal_temp_mspaint.Name = "_mspaint"
			end
	...
	elseif isRooms then
		local Rooms_AntiEntityGroupBox = Tabs.Floor:AddLeftGroupbox("Chống-quái")
		do
		...
		local Rooms_AutomationGroupBox = Tabs.Floor:AddRightGroupbox("")
		do
			...
			Rooms_AutomationGroupBox:AddLabel(
				"Cài đặt khuyên dùng:\nTắt xuyên tường và bỏ qua tốc độ",
				true
			)
		...
		function Script.Functions.GetAutoRoomsPathfindingGoal(): BasePart
			...
			if entity and entity.PrimaryPart.Position.Y > -10 then
				local GoalLocker = Script.Functions.GetNearestAssetWithCondition(function(asset)
					return asset.Name == "Rooms_Locker"
						and not asset.HiddenPlayer.Value
						and asset.PrimaryPart.Position.Y > -10
				end)
		...
		local _internal_mspaint_pathfinding_nodes = Instance.new("Folder", Workspace)
		do
			_internal_mspaint_pathfinding_nodes.Name = "_internal_mspaint_pathfinding_nodes"
		end

		local _internal_mspaint_pathfinding_block = Instance.new("Folder", Workspace)
		do
				...
				local function doAutoRooms()
					...
					if path.Status == Enum.PathStatus.Success then
						...
						for i, waypoint in pairs(waypoints) do
							...
							local waypointConnection = humanoid.MoveToFinished:Connect(function()
								moveToFinished = true
							end)
							if not moveToFinished or not Toggles.AutoRooms.Value then
								...
								if
									...
									and pathfindingGoal.Parent.Name ~= "Rooms_Locker"
								then
									...
									if not Toggles.AutoRooms.Value then
										...
										if
											_internal_mspaint_pathfinding_nodes:FindFirstChild("_internal_node_" .. i)
										then
											_internal_mspaint_pathfinding_nodes
												:FindFirstChild("_internal_node_" .. i)
												:Destroy()
										end
									end
								...
								task.delay(1.5, function()
									...
									if not Toggles.AutoRooms.Value or Library.Unloaded then
										return moveToCleanup()
									end

									repeat
										task.wait(0.25)
									until not character:GetAttribute("Hiding") and not character.PrimaryPart.Anchored
							...
							repeat
								task.wait()
							until moveToFinished or not Toggles.AutoRooms.Value or recalculate or Library.Unloaded
	...
	elseif isFools then
		local Fools_TrollingGroupBox = Tabs.Floor:AddLeftGroupbox("Trolling")
		do
			...
			function Script.Functions.ThrowBananaJeff()
				...
				local spawnPos = camera.CFrame:ToWorldSpace(
					CFrame.new(0, 0, -3) * CFrame.lookAt(Vector3.new(0, 0, 0), camera.CFrame.LookVector)
				)
		...
		local Fools_AntiEntityGroupBox = Tabs.Floor:AddRightGroupbox("Anti-Entity")
		do
		...
		local Fools_BypassGroupBox = Tabs.Floor:AddRightGroupbox("Bypass")
		do
				...
				for _, jeff in pairs(workspace:GetChildren()) do
					if jeff:IsA("Model") and jeff.Name == "JeffTheKiller" then
						task.spawn(function()
							repeat
								task.wait()
							until isnetowner(jeff.PrimaryPart)
		...
		Toggles.GodmodeNoclipBypassFools:OnChanged(function(value)
			if value and humanoid and collision then
				...
				task.spawn(function()
					repeat
						task.wait()
					until not Toggles.GodmodeNoclipBypassFools.Value
					humanoid.HipHeight = 3.01
					task.wait()
					collision.Position = collision.Position + Vector3.new(0, 8, 0)
					task.wait()
		...
		Toggles.FigureGodmodeFools:OnChanged(function(value)
			if value and not Toggles.GodmodeNoclipBypassFools.Value then
				Toggles.GodmodeNoclipBypassFools:SetValue(true)
				Script.Functions.Alert({
					Title = "Figure Godmode",
					Description = "Godmode/Noclip Bypass is required to use figure godmode",
					Reason = "Godmode/Noclip Bypass not enabled",
				})
			end
...
Toggles.EnableJump:OnChanged(function(value)
	...
	if
		not value
		and not Toggles.SpeedBypass.Value
		and Options.SpeedSlider.Max ~= 7
		and not Script.FakeRevive.Enabled
	then
		Options.SpeedSlider:SetMax(7)
	end
end)
...
Toggles.AntiDupe:OnChanged(function(value)
	for _, room in pairs(workspace.CurrentRooms:GetChildren()) do
		for _, dupeRoom in pairs(room:GetChildren()) do
			if
				dupeRoom:GetAttribute("LoadModule") == "DupeRoom"
				or dupeRoom:GetAttribute("LoadModule") == "SpaceSideroom"
			then
				task.spawn(function()
					Script.Functions.DisableDupe(
						dupeRoom,
						value,
						dupeRoom:GetAttribute("LoadModule") == "SpaceSideroom"
					)
				end)
			end
		end
...
Toggles.UpsideDown:OnChanged(function(value)
	...
	if value then
		...
		task.spawn(function()
			repeat
				task.wait()
			until not Toggles.UpsideDown.Value or Library.Unloaded
	...
	if value and alive and character and not Script.FakeRevive.Enabled then
		...
		if oxygenModule and healthModule then
			task.delay(0.5, function()
				if not Toggles.FakeRevive.Value then
					return
				end
		...
		for _, hotbarItem in pairs(mainUI.MainFrame.Hotbar:GetChildren()) do
			if not hotbarItem:IsA("TextButton") then
				continue
			end
		...
		do
			...
			local determined_cframe = rootPart.CFrame
				* CFrame.new(0, 0, 0)
		...
		for _, part in pairs(character:GetDescendants()) do
			if
				...
				and part.Name ~= "Collision"
				and part.Parent.Name ~= "Collision"
			then
				--v.CanCollide = false
				part.Massless = true
		...
		task.spawn(function()
			...
			local walk, idle =
				previewCharacter.Humanoid:LoadAnimation(anims.Forward),
				previewCharacter.Humanoid:LoadAnimation(anims.Idle)
				...
				repeat
					...
					if character:GetAttribute("Crouching") then
						if not crouch.isPlaying then
							crouch:Play()
							crouch:AdjustSpeed(oldCrouchSpeed)
						end
			...
			if character:WaitForChild("RightHand", math.huge) then
				...
				Script.FakeRevive.Connections["ToolAnimHandler"] = character.ChildAdded:Connect(function(tool)
					if tool:IsA("Tool") then
						...
						if toolsAnim.idle then
							toolsAnim.idle:Play(0.4, 1, 1)
						end
						if toolsAnim.equip then
							toolsAnim.equip:Play(0.05, 1, 1)
						end
						...
						tool.Activated:Connect(function()
							if table.find(doorInteractables, tool.Name) then
								return
							end
				...
				Script.FakeRevive.Connections["ToolAnimHandler2"] = ProximityPromptService.PromptButtonHoldBegan:Connect(
					function(prompt)
						if
							(currentTool and table.find(doorInteractables, currentTool.Name))
							and (prompt.Name == "UnlockPrompt" and prompt.Parent.Name == "Lock")
						then
							holding = true
							holdStart = tick()
							startDurability = currentTool:GetAttribute("Durability")
				...
				Script.FakeRevive.Connections["ToolAnimInteractHandler"] = ProximityPromptService.PromptButtonHoldEnded:Connect(
					function(prompt)
						if
							(currentTool and table.find(doorInteractables, currentTool.Name))
							and (prompt.Name == "UnlockPrompt" and prompt.Parent.Name == "Lock")
		...
		local function generateCharacterCFrame(obj)
			...
			return CFrame.new(
				obj_pos,
				obj_pos - (Vector3.new(camera.CFrame.Position.X, obj_pos.Y, camera.CFrame.Position.Z) - obj_pos).unit
			)
		end
		...
		local function usePreviewCharacter(doStepped)
			...
			if doStepped ~= false then
				Library:Notify(
					"You are not longer visible to others because you have lost Network Ownership of your character.",
					5
				)
			...
			if previewCharacter:FindFirstChild("Humanoid") then
				previewCharacter.Humanoid.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None
			end
			...
			Library:GiveSignal(RunService.RenderStepped:Connect(function()
				if doStepped ~= false then
					previewCharacter:PivotTo(generateCharacterCFrame(character:GetPivot()))
				end
				if rootPart then
		...
		Script.FakeRevive.Connections["mainStepped"] = RunService.RenderStepped:Connect(function()
			...
			if character:FindFirstChild("Humanoid") then
				character.Humanoid.WalkSpeed = 15 + Options.SpeedSlider.Value
			end
			...
			if not character:FindFirstChild("HumanoidRootPart") then
				Library:Notify(
					"You have completely lost Network Ownership of your character which resulted of breaking Fake Death.",
					5
				)
			...
			for _, part in ipairs(character:GetDescendants()) do
				if part:IsA("BasePart") then
					if part.Name == "RagdollCollision" then
						part.CFrame = (
							charPartCFrames[part.Parent.Name .. part.Parent.ClassName] - Vector3.new(0, 1000, 0)
						)
		...
		if mainGameSrc then
			mainGameSrc.dead = false
		end

		ProximityPromptService.Enabled = true
		Script.FakeRevive.Connections["ProximityPromptEnabler"] = ProximityPromptService
		...
		for _, room in pairs(workspace.CurrentRooms:GetChildren()) do
			task.spawn(function()
				...
				if roomDetectPart then
					roomDetectPart.Size =
						Vector3.new(roomDetectPart.Size.X, roomDetectPart.Size.Y * 250, roomDetectPart.Size.Z)
		...
		Script.FakeRevive.Connections["CurrentRoomFix"] = workspace.CurrentRooms.ChildAdded:Connect(function(room)
			...
			if roomDetectPart then
				roomDetectPart.Size =
					Vector3.new(roomDetectPart.Size.X, roomDetectPart.Size.Y * 100, roomDetectPart.Size.Z)
	...
	if value then
		...
		if currentRoomModel then
			for _, wardrobe in pairs(currentRoomModel:GetDescendants()) do
				if
					wardrobe:GetAttribute("LoadModule") == "Wardrobe"
					or wardrobe:GetAttribute("LoadModule") == "Bed"
					or wardrobe.Name == "Rooms_Locker"
					or wardrobe.Name == "RetroWardrobe"
				then
...
Toggles.NoCutscenes:OnChanged(function(value)
	...
	if floorReplicated then
		for _, cutscene in pairs(floorReplicated:GetChildren()) do
			if not cutscene:IsA("ModuleScript") or table.find(CutsceneExclude, cutscene.Name) then
				continue
			end
...
if isBackdoor then
	...
	Library:GiveSignal(clientRemote.Haste.Remote.OnClientEvent:Connect(function(value)
		if not value and Toggles.NotifyEntity.Value then
			haste_incoming_progress = Instance.new("Part", Workspace)
			do
			...
			repeat
				task.wait()
			until not haste_incoming_progress or not Toggles.NotifyEntity.Value or not character:GetAttribute("Alive")
			if haste_incoming_progress then
...
Library:GiveSignal(ProximityPromptService.PromptTriggered:Connect(function(prompt, player)
	if player ~= localPlayer or not character or isFools then
		return
	end

	local isDoorLock = prompt.Name == "UnlockPrompt"
		and prompt.Parent.Name == "Lock"
		and not prompt.Parent.Parent:GetAttribute("Opened")
	local isSkeletonDoor = prompt.Name == "SkullPrompt"
		and prompt.Parent.Name == "SkullLock"
	...
	local isChestBox = prompt.Name == "ActivateEventPrompt"
		and prompt.Parent.Name == "ChestBoxLocked"
...
Library:GiveSignal(workspace.ChildAdded:Connect(function(child)
	task.delay(0.1, function()
				...
				if child:IsDescendantOf(workspace) then
					...
					if table.find(EntityTable.AutoWardrobe.Entities, child.Name) then
						...
						task.spawn(function()
							repeat
								RunService.Heartbeat:Wait()
							until not child:IsDescendantOf(workspace)
								or Script.Functions.DistanceFromCharacter(child) <= distance
					...
					if Options.NotifyEntity.Value[shortName] then
						Script.Functions.Alert({
							...
							Reason = (
								not EntityTable.NotifyReason[child.Name].Spawned and "Hãy tìm nơi ẩn náu!" or nil
							),
		...
		if isFools then
			...
			if Toggles.AntiJeffServer.Value and child.Name == "JeffTheKiller" then
				task.spawn(function()
					repeat
						task.wait()
					until isnetowner(child.PrimaryPart)
		...
		if
			(child.Name == "RushMoving" or child.Name == "AmbushMoving")
			and Toggles.InfCrucifix.Value
			and alive
			and character
		then
			task.wait(1.5)
			...
			crucifixConnection = RunService.RenderStepped:Connect(function(deltaTime)
				if not Toggles.InfCrucifix.Value or not alive or not character then
				...
				local inView =
					Script.Functions.IsInViewOfPlayer(child, EntityTable.InfCrucifixVelocity[entityName].minDistance)
					...
					if character:FindFirstChild("Crucifix") then
						workspace.Drops.ChildAdded:Once(function(droppedItem)
							if droppedItem.Name == "Crucifix" then
								local targetProximityPrompt = droppedItem:WaitForChild("ModulePrompt", 3)
									or droppedItem:FindFirstChildOfClass("ProximityPrompt")
								repeat
									task.wait()
			...
			childRemovedConnection = workspace.ChildRemoved:Connect(function(model: Model)
				if model ~= child then
					return
				end
...
Library:GiveSignal(localPlayer.OnTeleport:Connect(function(state)
	if
		(state == Enum.TeleportState.RequestedFromServer or state == Enum.TeleportState.Started)
		and Toggles.ExecuteOnTeleport.Value
		and not getgenv().queued_to_teleport
	then
	...
	if currentRoomModel then
		for _, asset in pairs(currentRoomModel:GetDescendants()) do
			...
			if
				...
				and (asset:GetAttribute("Storage") == "ChestBox" or asset.Name == "Toolshed_Small")
			then
				task.spawn(Script.Functions.ChestESP, asset)
			end

			if
				Toggles.HidingSpotESP.Value
				and (
					asset:GetAttribute("LoadModule") == "Wardrobe"
					or asset:GetAttribute("LoadModule") == "Bed"
					or asset.Name == "Rooms_Locker"
					or asset.Name == "RetroWardrobe"
				)
			then
...
Library:GiveSignal(UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if
		isFools
		and Library.IsMobile
		and input.UserInputType == Enum.UserInputType.Touch
		and Toggles.GrabBananaJeffToggle.Value
	then
		...
		if target and isnetowner(target) then
			if target.Name == "BananaPeel" then
				...
				if not target:GetAttribute("Clip") then
					target:SetAttribute("Clip", target.CanCollide)
				end
			...
			elseif target:FindFirstAncestorWhichIsA("Model").Name == "JeffTheKiller" then
				...
				for _, i in ipairs(jeff:GetDescendants()) do
					if i:IsA("BasePart") then
						if not i:GetAttribute("Clip") then
							i:SetAttribute("Clip", target.CanCollide)
						end
...
Library:GiveSignal(RunService.RenderStepped:Connect(function()
	...
	if character then
		if
			...
			and not (mainGameSrc and mainGameSrc.stopcam or rootPart.Anchored and not character:GetAttribute("Hiding"))
		then
		...
		if Toggles.AutoInteract.Value and (Library.IsMobile or Options.AutoInteractKey:GetState()) then
			local prompts = Script.Functions.GetAllPromptsWithCondition(function(prompt)
				...
				if
					prompt.Parent:GetAttribute("PropType") == "Battery"
					and (
						(
							character:FindFirstChildOfClass("Tool")
							and character:FindFirstChildOfClass("Tool"):GetAttribute("RechargeProp") ~= "Battery"
				...
				if
					prompt.Parent:GetAttribute("PropType") == "Heal"
					and humanoid
			...
			for _, prompt: ProximityPrompt in pairs(prompts) do
				task.spawn(function()
					...
					if
						Script.Functions.DistanceFromCharacter(prompt.Parent) < prompt.MaxActivationDistance
						and (
							not prompt:GetAttribute("Interactions" .. localPlayer.Name)
							or PromptTable.Aura[prompt.Name]
							or table.find(PromptTable.AuraObjects, prompt.Parent.Name)
						)
					then
						-- painting checks
						if prompt.Parent.Name == "Slot" and prompt.Parent:GetAttribute("Hint") and character then
							if Script.Temp.PaintingDebounce then
								return
							end
							...
							if
								not currentPainting
								and prompt.Parent:FindFirstChild("Prop")
								and prompt.Parent:GetAttribute("Hint") ~= prompt.Parent.Prop:GetAttribute("Hint")
							then
							...
							if prompt.Parent:GetAttribute("Hint") == currentPainting:GetAttribute("Hint") then
								...
								repeat
									task.wait()
									firePrompt(prompt)
								until not character:FindFirstChild("Prop")
									or character:FindFirstChild("Prop"):GetAttribute("Hint") ~= oldHint
		...
		if isMines then
			if
				Toggles.AutoAnchorSolver.Value
				and latestRoom.Value == 50
				and mainUI.MainFrame:FindFirstChild("AnchorHintFrame")
			then
				local prompts = Script.Functions.GetAllPromptsWithCondition(function(prompt)
					return prompt.Name == "ActivateEventPrompt"
						and prompt.Parent:IsA("Model")
						and prompt.Parent.Name == "MinesAnchor"
						and not prompt.Parent:GetAttribute("Activated")
				end)
				...
				for _, prompt in pairs(prompts) do
					task.spawn(function()
						...
						if
							not (Script.Functions.DistanceFromCharacter(prompt.Parent) < prompt.MaxActivationDistance)
						then
							return
						end
						...
						local result = Anchor:FindFirstChildOfClass("RemoteFunction")
							:InvokeServer(CurrentGameState.AnchorCode)
						if result then
							Script.Functions.Alert({
		...
		if
			Toggles.AntiEyes.Value and (workspace:FindFirstChild("Eyes") or workspace:FindFirstChild("BackdoorLookman"))
		then
			if not isFools then
...
Library:OnUnload(function()
	-- disconnect hook
	if mtHook then
		hookmetamethod(game, "__namecall", mtHook)
	end
	if _fixDistanceFromCharacter then
		hookmetamethod(localPlayer, "__namecall", _fixDistanceFromCharacter)
	end
	...
	if isMines then
		...
		if acbypasspart then
			acbypasspart:Destroy()
		end
	end
	...
	if collision then
		collision.CanCollide = if mainGameSrc
			then not mainGameSrc.crouching
			else not character:GetAttribute("Crouching")
		if collision:FindFirstChild("CollisionCrouch") then
			collision.CollisionCrouch.CanCollide = if mainGameSrc
				then mainGameSrc.crouching
				else character:GetAttribute("Crouching")
		end
	end
	...
	for _, antiBridge in pairs(Script.Temp.Bridges) do
		antiBridge:Destroy()
	end
...
MenuGroup:AddToggle(
	"ExecuteOnTeleport",
	{ Default = false, Text = "Execute on Teleport", Visible = ExecutorSupport["queue_on_teleport"] }
)
